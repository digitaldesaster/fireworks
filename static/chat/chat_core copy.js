...
>>>>>>> Stashed changes
    ...
    // Check for ordered or unordered list items
    const olMatch = line.match(/^(\s*)(\d+)\.\s+(.*)$/);
    const ulMatch = line.match(/^(\s*)[\*\-]\s+(.*)$/);
    
    if (olMatch || ulMatch) {
      const isOrdered = !!olMatch;
      const content = isOrdered ? olMatch[3] : ulMatch[2];

      // Convert tabs to four spaces for indent calculation
      const indentStr = (isOrdered ? olMatch[1] : ulMatch[1]).replace(/\t/g, "   ");
      // Each 4 spaces = 1 indent level
      const indentLevel = Math.floor(indentStr.length / 4);

      // If there's no current list or our indent level changed, create a new list
      if (!currentList || indentLevel !== currentListLevel) {
        finishCurrentList();
        currentList = document.createElement(isOrdered ? "ol" : "ul");
        currentList.className = `list-inside ${
          isOrdered ? "list-decimal" : "list-disc"
        } marker:text-gray-500 ${indentLevel > 0 ? "pl-6" : ""}`; 
        currentListLevel = indentLevel;

        // If indentLevel > 0, nest this within the last list item at the previous level
        if (indentLevel > 0) {
          const parentList = listStack[indentLevel - 1];
          if (parentList && parentList.lastElementChild) {
            // Create a small container div to hold the nested list
            const nestedDiv = document.createElement("div");
            nestedDiv.className = "mt-1";
            nestedDiv.appendChild(currentList);
            parentList.lastElementChild.appendChild(nestedDiv);
          } else {
        ...
        // Track this new list in the stack
        listStack[indentLevel] = currentList;
        // Trim any deeper levels beyond our current indent
        listStack.length = indentLevel + 1;
      }
      // If the list type (ul/ol) changed at the same indent level, close out the old list, open a new one
      else if (isOrdered !== (currentList.tagName === "OL")) {
        finishCurrentList();
        currentList = document.createElement(isOrdered ? "ol" : "ul");
        currentList.className = `list-inside ${
          isOrdered ? "list-decimal" : "list-disc"
        } marker:text-gray-500 ${currentListLevel > 0 ? "pl-6" : ""}`;
        container.appendChild(currentList);
        listStack[currentListLevel] = currentList;
      }

      // Create the list item
      const li = document.createElement("li");
      li.className = "mb-2";
      li.innerHTML = processInlineMarkdown(content);
      currentList.appendChild(li);

      return;
    }

    // If we encounter a non-list line, close the current list
    finishCurrentList();
...
=======
  // Helper to close out the current list
  function finishCurrentList() {
    if (currentList) {
      // If the list is top-level and not attached, attach it
      if (currentListLevel === 0 && currentList.parentElement === null) {
        container.appendChild(currentList);
      }
      currentList = null;
      currentListLevel = 0;
